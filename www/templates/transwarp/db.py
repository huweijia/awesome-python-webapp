...
class _ConnectionCtx(object):
    '''
    _ConnectionCtx object that can open anc close connection context._ConnectionCtx object can be nested and only the most outer connection has effect.
    with connection():
        pass
        with connection():
            pass
...
def with_transaction(func):
    ...
    u'JULIA'
    >>> update_profile(8080,'Robert',True)
    Traceback(most recent call last):
        ...
    StandardError:will cause rollback...
...
def select_one(sql,*args):
    ...
    if multiple resuls found ,the first one returned.
    ...
    >>> u.name
    u'Alice'
    >>> select_one('seelct * from user where email=?','abc@email.com')
...
def select_int(sql,*args):
    ...
    >>> u1 = dict(id=96900,name='Ada',email='ada@test.org',passwd='A-12345',last-modified=time.time())
    >>> u2=dict(id=96901,name='Adam',email='adam@test.org',passwd='A-12345',last-modified=time.time())
    >>> insert('user',**u1)
    1
    >>> insert('user',**u2)
    1
