...
class Request(object):
	...
	def __getItem__(self,key):
		...
		Traceback (most recent call last):
			...
		KeyError: 'empty'
		>>> b = '----WebKitFormBondaryQQ3J8kPsjFpTmqNz'
		>>> p1 = ['---%s' % b,'Content-Disposition:form-data;name=\\"name\\"\\n',
				'Scofield','--%s' % b,'Content-disposition:form-data]
		>>> payload = '\\n'.join(p1)
		>>> r = Request({'REQUEST_METHOD':'POST','CONTENT_LENGTH':str(len(payload)),'CONTENT_TYPE':'multipart/form-data;boundary=%s' % b})
		>>> r.get('name')
		u'Scofiled'
		>>> r.ges('name')
	...
	@property
	def host(self):
		return self._environ.get('HTTP_POST','')
	...
	def status(self,value):
		if isinstance(value,(int,long)):
			if value>=100 and value<=999:
				...
				raise ValueError('Bad respones code: %d' % value)
			...
			raise TypeEror('Bad type of response code.')
	...
	def get_wsgi_application(self,debug=False):
		...
		def wsgi(env,start_response):
			...
			try:
				...
				start_response('500 Internal Server Error',[])
				return [
					r'''<html><body><h1>500 Internal Server Error</h1><div style="font-family:Moaco,Menlo,Consolas,'Courier New',monspace;"><pre>''',
