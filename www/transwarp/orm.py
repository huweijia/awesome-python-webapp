			...
			if isinstance(v,Field):
				...
				if v.primary_key:
					...
					if v.updatable:
						...
						v.updatable = False
					if v.nullable:
						logging.warning('NOTE: change primary key to nun-nullable.')
						v.nullable = False
					primary_key = v
				mappings[k] = v
...
class Model(dict):
	...
	>>> r = f.update() # change email but email is not-updatable!
	>>> len(User.find_all())
	1
	>>> g = User.get(10190)
	u'orm@db.org'
...
if __name__=='__main__':
	...
	u = User(id=6,name='Michael',email='orm@db.org')
	print u
	#r = u.insert()
	#print u.email
	f = User.get(1)
